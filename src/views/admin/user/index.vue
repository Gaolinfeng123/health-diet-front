...
<template>
  <div class="admin-user-container">
    <el-card shadow="never">
      <div class="header">
        <div class="left">
          <el-input v-model="queryParams.username" placeholder="搜索用户名" clearable @clear="loadData" @keyup.enter="loadData" style="width: 200px" />
    ...
    <el-card shadow="never" class="mt-20">
      <el-table :data="tableData" v-loading="loading" stripe>
        ...
        <el-table-column label="角色" width="120"><template #default="s"><el-tag :type="s.row.role===1?'danger':'info'">{{s.row.role===1?'管理员':'普通用户'}}</el-tag></template></el-table-column>
      ...
      <div class="pagination-box" v-if="total > 0"><el-pagination background layout="prev, pager, next" :total="total" :page-size="queryParams.pageSize" @current-change="handlePageChange"/></div>
    </el-card>
...
.header { display: flex; justify-content: space-between; } .left { display:flex; gap:10px; }
.mt-20 { margin-top: 20px; } .pagination-box { margin-top:15px; display:flex; justify-content:flex-end; }
</style>